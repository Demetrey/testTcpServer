...
CurrentTask::CurrentTask(QObject *parent) : QObject(parent)
{
    _interval = 30000;
}
...
void CurrentTask::run()
{
    ...
    {
        std::cout << "Client address: " << this->_socket->peerAddress().toString().toStdString() << std::endl;
        std::cout << "Client port: " << this->_socket->peerPort() << std::endl;
        connect(_socket, SIGNAL(disconnected()), this, SLOT(onDisconnected()), Qt::DirectConnection);
        connect(_socket, SIGNAL(readyRead()), this, SLOT(onReadyRead()), Qt::DirectConnection);

        connect(this->_timer, SIGNAL(timeout()), this, SLOT(onTimerCompleted()), Qt::DirectConnection);
    ...
    std::cout << "Create new thread on " << _socketDescriptor << std::endl;
    _eventLoop->exec(); // The event loop starts, the corresponding slots are executed
